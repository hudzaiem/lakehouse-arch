...
curl -fSL -o "$LIB_DIR/flink-sql-connector-kafka-3.3.0-${FLINK_VERSION}.jar" \
  "https://repo1.maven.org/maven2/org/apache/flink/flink-sql-connector-kafka/3.3.0-${FLINK_VERSION}/flink-sql-connector-kafka-3.3.0-${FLINK_VERSION}.jar"

# 2. Iceberg Flink Runtime
echo "[2/4] iceberg-flink-runtime..."
curl -fSL -o "$LIB_DIR/iceberg-flink-runtime-${FLINK_VERSION}-${ICEBERG_VERSION}.jar" \
...
curl -fSL -o "$LIB_DIR/gcs-connector-hadoop3-2.2.22-shaded.jar" \
  "https://storage.googleapis.com/hadoop-lib/gcs/gcs-connector-hadoop3-2.2.22-shaded.jar"
